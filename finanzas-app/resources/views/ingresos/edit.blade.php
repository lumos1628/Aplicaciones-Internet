<x-app-layout>
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <h1 class="text-2xl font-semibold text-gray-900 mb-6">Editar Ingreso</h1>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <form action="{{ route('ingresos.update', $ingreso) }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    <select name="consumidorId" id="consumidorId"
                        ...
                        @foreach($consumidores as $consumidor)
                            <option value="{{ $consumidor->id }}" {{ old('consumidorId', $ingreso->consumidorId) == $consumidor->id ? 'selected' : '' }}>
                                {{ $consumidor->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-5">
                    ...
                    <input type="number" name="monto" id="monto" value="{{ old('monto', $ingreso->monto) }}" step="0.01" min="0"
                           class="w-full rounded-lg border-gray-300 shadow-sm focus:border-slate-700 focus:ring focus:ring-slate-200 focus:ring-opacity-50">
                </div>

                <div class="mb-5">
                    <label for="fecha" class="block text-sm font-medium text-gray-700 mb-1">Fecha</label>
                    <input type="date" name="fecha" id="fecha" value="{{ old('fecha', $ingreso->fecha) }}"
                           class="w-full rounded-lg border-gray-300 shadow-sm focus:border-slate-700 focus:ring focus:ring-slate-200 focus:ring-opacity-50">
                </div>

                <div class="mb-6">
                    <label for="descripcion" class="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
                    <input type="text" name="descripcion" id="descripcion" value="{{ old('descripcion', $ingreso->descripcion) }}"
                           class="w-full rounded-lg border-gray-300 shadow-sm focus:border-slate-700 focus:ring focus:ring-slate-200 focus:ring-opacity-50">
                </div>
